...
function lakito:update(dt)
	...
	elseif self.passive then
		...
		for i = 2, players do
			...
			if v.x > objects["player"][nearestplayer].x and not v.dead then
				nearestplayer = i
		...
		if not (onlinemp and clientisnetworkhost or not onlinemp) then
			return false
		end
		...
		if count < 3 then
			if self.timer > lakitothrowtime then
				...
				table.insert(networksendqueue, "spikethrow;" .. networkclientnumber .. ";" .. self.x+6/16 .. ";" .. self.y)
			end
		end
...
function lakito:shotted() --fireball, star, turtle
	...
	self.speedx = 0
	self.timer = 0
	sendobjectshotted(self, "lakito", "right")
end
