...
function goomba:stomp(relayed)--hehe goomba stomp
	self.dead = true
	self.active = false
	self.quad = goombaquad[spriteset][2]
	if not relayed then
		table.insert(networksendqueue, "goombakill;" .. networkclientnumber .. ";" .. self.x .. ";" .. self.y .. ";stomp")
	end
end

function goomba:shotted(dir, relayed) --fireball, star, turtle
	playsound(shotsound)
	self.shot = true
	...
	if not relayed then
		table.insert(networksendqueue, "goombakill;" .. networkclientnumber .. ";" .. self.x .. ";" .. self.y .. ";shot")
	end
end
	...
	if self.t == "spikeyfall" then
		...
		if clientisnetworkhost then
			...
			if self.x >= nearestplayerx then
				...
				table.insert(networksendqueue, "spiketurn;" .. networkclientnumber .. ";" .. number)
				self.speedx = goombaspeed
				self.animationdirection = "left"
			end
		end
