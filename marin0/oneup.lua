...
function oneup:draw()
	if self.drawable == false then
		...
		love.graphics.draw(entitiesimg, entityquads[3].quad, math.floor(((self.x-xscroll)*16+self.offsetX)*scale), math.floor(((self.y-yscroll)*16-self.offsetY)*scale), 0, scale, scale, self.quadcenterX, self.quadcenterY)
	end
end
...
function givelive(id, t)
	...
	if onlinemp then
		local string = "1up;" .. networkclientnumber .. ";"
		for i = 1, players do
			string = string .. mariolives[i] .. ";"
		end

		table.insert(networksendqueue, string .. math.floor(t.x) .. ";" .. math.floor(t.y))
	end
end	
