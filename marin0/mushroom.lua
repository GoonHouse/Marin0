...
function mushroom:draw()
	if self.uptimer < mushroomtime and not self.destroy then
		...
		love.graphics.draw(entitiesimg, entityquads[2].quad, math.floor(((self.x-xscroll)*16+self.offsetX)*scale), math.floor(((self.y-yscroll)*16-self.offsetY)*scale), 0, scale, scale, self.quadcenterX, self.quadcenterY)
	end
end
