	...
	if (onlinemp and clientisnetworkhost or not onlinemp) then
		...
		if self.timer > platformspawndelay then
			if self.direction == "up" then
				...
				table.insert(networksendqueue, "createpform;" .. networkclientnumber .. ";" .. self.x .. ";" .. self.y+1 .. ";justup;" .. self.size)
			else
				table.insert(objects["platform"], platform:new(self.x, self.y-0.5, "justdown", self.size))
				table.insert(networksendqueue, "createpform;" .. networkclientnumber .. ";" .. self.x .. ";" .. self.y-.5 .. ";justdown;" .. self.size)
			end
