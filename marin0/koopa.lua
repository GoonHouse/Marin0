...
function koopa:stomp(x, b)
	if onlinemp and b.playernumber == 1 then
		local string = "koopastomp;" .. networkclientnumber .. ";" .. math.floor(self.x) .. ";" .. math.floor(self.y)
		if x then
			string = string .. ";" .. x
		end
		table.insert(networksendqueue, string)
	end
	if self.flying then
		self.flying = false
		self.quad = koopaquad[spriteset][1]
...
function koopa:shotted(dir, relayed) --fireball, star, turtle
	playsound(shotsound)
	self.shot = true
	self.small = true
	self.quad = koopaquad[spriteset][3]
	...
	if not relayed then
		sendobjectshotted(self, "koopa", self.direction)
	end
