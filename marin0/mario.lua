...
local networkspawndelaytimer = 0
...
function mario:init(x, y, i, animation, size, t)
	...
	elseif classicmodeactive then
		for j = 0, 3 do
			self.smallgraphic[j] = classicmarioanimations[j]
		end
	else
		for j = 0, 3 do
	...
	elseif classicmodeactive then
		for j = 0, 3 do
			self.biggraphic[j] = bigclassicmarioanimations[j]
		end
	else
		for j = 0, 3 do
	...
	local colorthing = self.playernumber
	if self.playernumber == 1 then
		colorthing = playerconfig
	end
	self.colors = mariocolors[colorthing]
	if self.size == 1 then
		self.offsetX = 6
		self.offsetY = 3
		self.quadcenterX = 11
	...
	--hat
	self.hats = mariohats[colorthing]
	self.drawhat = true
	...
	if players == 1 then
		...
		self.portal1color = portalcolor[colorthing][1]
		self.portal2color = portalcolor[colorthing][2]
	end
	...
	if self.animation == "pipeup" then
		...
		if self:canperformanimation() then
...
function mario:update(dt)
	...
	networkspawndelaytimer = networkspawndelaytimer + dt
	self.passivemoved = false
	...
	if self.startimer < mariostarduration and not self.dead then
		...
		if self.startimer >= mariostarduration then
			if self.size == 3 then --flower colors
				...
				self.colors = mariocolors[playerconfig]
			end
			self.starred = false
			self.startimer = mariostarduration
		end
	...
	if self.animation == "pipedown" then
		...
		if self.animationtimer < pipeanimationtime then
			...
			if self.animationtimer >= pipeanimationtime+pipeanimationdelay then
				...
				elseif self.animationmisc then --warpzone
	...
	elseif self.animation == "vine" then
		...
		if self.y < -4 and not levelfinished then
	...
	if self.springgreen then
		...
		if self.springgreentimer > springgreentime then
	...
	--coins
	if not editormode and self.playernumber == 1 then
		...
		if inmap(x, y) and tilequads[map[x][y][1]].coin then
			collectcoin(x, y)
			table.insert(networksendqueue, "coin;" .. networkclientnumber .. ";" .. x .. ";" .. y)
		end
	...
	local x = math.floor(self.x+self.width/2)+1
	local y = math.floor(self.y+self.height/2)+1
	local collidedwithmazegate = false

	if inmap(x, y) and map[x][y][2] and entityquads[map[x][y][2]].t == "mazegate" and not map[x][y].nullifymaze then
		if map[x][y][3] == self.mazevar + 1 then
			local value = self.mazevar+1
			for x = 1, players do
				self.mazevar = value
			end
			table.insert(networksendqueue, "mazevar;" .. networkclientnumber .. ";" .. value)
		elseif map[x][y][3] == self.mazevar then
			
		else
			self.mazevar = 0
		end
		collidedwithmazegate = true
	end
	--Make it so worlds can't desync in online mp

	local removey = false
	if not collidedwithmazegate then
		for checky = 1, 15 do
			--passed over/underneath a mazegate
			if inmap(x, checky) and map[x][checky][2] and entityquads[map[x][checky][2]].t == "mazegate" then
				map[x][checky].nullifymaze = true
				--Make it so that those can't be checked, even if the person has passed it
			end
		end
	end
	...
	if classicmodeactive then
		self.playerangle = math.pi/8*4
	else
		if (self.controlsenabled and (onlinemp and self.playernumber == 1)) or singularmariogamemode or not onlinemp then
			self:updateangle()
		end
	end
	...
	if self.controlsenabled then
		...
		local test = true
		if onlinemp and self.playernumber ~= 1 then
			test = false
		end
		if self.falling == false and self.jumping == false and self.size > 1 and test then
		...
		--Real lazy fix, but whatever
		if firestartx and self.x >= firestartx - 1 and (onlinemp and clientisnetworkhost and networkspawndelaytimer > 10 or not onlinemp) then
			firestarted = true
		end

		if fireendx and self.x >= fireendx - 1 and (onlinemp and clientisnetworkhost and networkspawndelaytimer > 10 or not onlinemp) then
			firestarted = false
		end
		
		if flyingfishstartx and self.x >= flyingfishstartx - 1 and (onlinemp and clientisnetworkhost and networkspawndelaytimer > 5 or not onlinemp) then
			flyingfishstarted = true
		end
		
		if flyingfishendx and self.x >= flyingfishendx - 1 and (onlinemp and clientisnetworkhost and networkspawndelaytimer > 10 or not onlinemp) then
			flyingfishstarted = false
		end
		
		if bulletbillstartx and self.x >= bulletbillstartx - 1  and (onlinemp and clientisnetworkhost and networkspawndelaytimer > 5 or not onlinemp) then
			bulletbillstarted = true
		end
		
		if bulletbillendx and self.x >= bulletbillendx - 1 and (onlinemp and clientisnetworkhost and networkspawndelaytimer > 10 or not onlinemp)  then
			bulletbillstarted = false
		end
		
		if lakitoendx and self.x >= lakitoendx and (onlinemp and networkspawndelaytimer > 6 or not onlinemp) then
			lakitoend = true
		end
	...
	if onlinemp and self.playernumber ~= 1 and self.speedy < 0 then
		self.animationstate = "jumping"
		self.jumping = true
		self.falling = false
	elseif onlinemp and self.playernumber ~= 1 and self.onlightbridge then
		self.jumping = false
		self.falling = false
...
function mario:updateangle()
	...
	if (onlinemp and self.playernumber == 1 or not onlinemp) then
...
function mario:setquad(anim, s)
	...
	--[[if onlinemp then
		if self.mostrecentanimationsynced ~= self.animationstate then
			self.mostrecentanimationsynced = self.animationstate
			table.insert(networksendqueue, "animationstate;" .. networkclientnumber .. ";" .. self.animationstate)
		end
	end--]]
end

function mario:jump(relayed)
	if not noupdate and self.controlsenabled then
		if not underwater then
			...
			if self.falling == false or relayed then
				if self.size == 1 then
					playsound(jumpsound)
				else
...
function mario:floorcollide(a, b)
	...
	if a == "springgreen" then
		self:hitspringgreen(b)
		return false
	end

	if a == "lightbridgebody" or a == "lightbridge" then
		if self.playernumber ~= 1 then
			self.onlightbridge = true
		end
	else
		self.onlightbridge = false
	end
	...
	if a == "mushroom" or a == "oneup" or a == "star" or a == "flower" or a == "poisonmush" then
		self.falling = true
		return false
	elseif (a == "goomba" and b.t == "goomba") or a == "bulletbill" or a == "flyingfish" or a == "lakito" or a == "hammerbro" or a == "koopa" or a == "squid" then
		self:stompenemy(a, b)
		return false
	elseif a == "tile" then
		local x, y = b.cox, b.coy
	...
	elseif a == "plant" or a == "bowser" or a == "cheep" or a == "upfire" or (a == "goomba" and b.t ~= "goomba") or a == "squid" or a == "hammer" or a == "redplant"  or a == "reddownplant"  or a == "downplant" then --KILL
...
function mario:stompenemy(a, b, relayed)
	if self.playernumber ~= 1 and not relayed then
		return
	end
	local bounce = false
	if a == "koopa" then
...
function mario:rightcollide(a, b)
	...
	if a == "mushroom" or a == "oneup"or a == "poisonmush" or a == "star" or a == "flower" or (a == "platform" and (b.dir == "right" or b.dir == "justright")) then
		return false
	elseif self.speedy > 2 and ((a == "goomba" and b.t == "goomba") or a == "bulletbill" or a == "flyingfish" or a == "lakito" or a == "hammerbro" or a == "koopa") then
		self:stompenemy(a, b)
		return false
	elseif a == "castlefirefire" or a == "fire" or a == "koopa" or a == "goomba" or a == "bulletbill" or a == "plant" or a == "bowser" or a == "cheep" or a == "flyingfish" or a == "upfire" or a == "lakito" or a == "squid" or a == "hammer" or a == "hammerbro" or a == "redplant" or a == "reddownplant"  or a == "downplant"then --KILLS
		if self.invincible then
...
function mario:leftcollide(a, b)
	...
	if a == "mushroom" or a == "oneup" or a == "poisonmush" or a == "star" or a == "flower" or (a == "platform" and (b.dir == "right" or b.dir == "justright")) then --NOTHING
		return false
	elseif self.speedy > 2 and ((a == "goomba" and b.t == "goomba") or a == "bulletbill" or a == "flyingfish" or a == "lakito" or a == "hammerbro" or a == "koopa") then
		self:stompenemy(a, b)
		return false
...
function mario:ceilcollide(a, b)
	...
	if a == "mushroom" or a == "oneup" or a == "star" or a == "flower" or a == "poisonmush" then --STUFF THAT SHOULDN'T DO SHIT
		return false
	elseif a == "castlefirefire" or a == "fire" or a == "plant" or a == "goomba" or a == "koopa" or a == "bulletbill" or a == "bowser" or a == "cheep" or a == "flyingfish" or a == "upfire" or a == "lakito" or a == "squid" or a == "hammer" or a == "hammerbro" or a == "redplant"  or a == "reddownplant"  or a == "downplant" then --STUFF THAT KILLS
		if self.invincible then
			return false
		else
			self:die("Enemy")
...
function mario:starcollide(a, b)
	--enemies that die
	if a == "goomba" or a == "koopa" or a == "plant" or a == "bowser" or a == "squid" or a == "cheep" or a == "hammerbro" or a == "lakito" or a == "bulletbill" or a == "flyingfish" or a == "redplant"  or a == "reddownplant"  or a == "downplant" then
		b:shotted("right")
		if a ~= "bowser" then
...
function hitblock(x, y, t, relayed)
	local checkif1 = clientisnetworkhost
	local isturtle
	...
	if not checkif1 then
		if t.playernumber == 1 then
			checkif1 = true
		end
	end
	if not checkif1 and not relayed then
		return
	end
	...
	if onlinemp and not relayed then
		table.insert(networksendqueue, "hitblock;" .. networkclientnumber .. ";" .. x .. ";" .. y)
	end
end
...
function mario:die(how, relayed)

	local checkif1 = false
	if self.playernumber == 1 then
		checkif1 = true
	end
	...
	if onlinemp and not relayed then
		...
		table.insert(networksendqueue, "death;" .. networkclientnumber .. ";" .. how)
	end
	...
	everyonedead = true
	for i = 1, players do
		if not objects["player"][i].dead and not singularmariogamemode then
			everyonedead = false
		end
	end
	...
	if not levelfinished and not testlevel and mariolivecount ~= false then
		if not infinitelives then
			mariolives[self.playernumber] = mariolives[self.playernumber] - 1
		else
			print("adding death")
			mariodeaths[self.playernumber] = mariodeaths[self.playernumber] + 1
		end
	end
...
function mario:removeportals(i)
	...
	if onlinemp then
		if self.playernumber == 1 then
			table.insert(networksendqueue, "reload;" .. networkclientnumber)
		end
	end
end

function mario:use()
	if self.playernumber == 1 and onlinemp then
		--Send pointingangle first, hopefully
		udp:send("pointingangle;" .. networkclientnumber .. ";" .. round(objects["player"][1].pointingangle, 2) .. ";now")
		table.insert(networksendqueue, "use;" .. networkclientnumber)
	end
	if self.pickup then
...
function mario:duck(ducking) --goose
	...
	if onlinemp then
		if self.playernumber == 1 then
			...
			table.insert(networksendqueue, "duck;" .. networkclientnumber .. ";" .. string)
		end
	end
end
...
function mario:pipe(x, y, dir, i)
	...
	if onlinemp and self.playernumber == 1 then
		table.insert(networksendqueue, "pipe;" .. networkclientnumber .. ";" .. x .. ";" .. y .. ";" .. dir .. ";" .. i)
	end
	
	self:setquad()
end

function mario:flag(relayed)
	if levelfinished or (self.playernumber ~= 1 and not relayed) then
		return
	end
	self.ducking = false
	self.animation = "flag"
	...
	if self.playernumber == 1 then
		table.insert(networksendqueue, "finish;" .. networkclientnumber .. ";" .. mariotime .. ";flag")
	end
	
	playsound(levelendsound)
end

function mario:axe(relayed)
	if levelfinished or (self.playernumber ~= 1 and not relayed) then
		return
	end
	self.ducking = false
	...
	if self.playernumber == 1 then
		table.insert(networksendqueue, "finish;" .. networkclientnumber .. ";" .. mariotime .. ";axe")
	end
...
function mario:star(starobject, relayed)
	addpoints(1000)
	self.startimer = 0
	...
	if not relayed then
		table.insert(networksendqueue, "powerup;" .. networkclientnumber .. ";" .. starobject.x .. ";" .. starobject.y .. ";star")
	end
end

function mario:fire(x, y, relayeddir)
	if not noupdate and self.controlsenabled and self.size == 3 and self.ducking == false then
		if self.fireballcount < maxfireballs then
			...
			local calculateddir = relayeddir
			if not calculateddir then
				calculateddir = "right"
				if self.pointingangle > 0 then
					calculateddir = "left"
				end
			end
			table.insert(objects["fireball"], fireball:new(x+.5, y, calculateddir, self))
			if onlinemp and self.playernumber == 1 then
				--Add dir just in case they whipped around and pointing angle hasn't been updated yet
				table.insert(networksendqueue, "fireball;" .. networkclientnumber .. ";" .. self.x .. ";" .. self.y .. ";" .. calculateddir)
			end
...
function mario:canperformanimation()
	return true
end
