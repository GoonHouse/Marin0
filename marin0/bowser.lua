...
function bowser:update(dt)
	if self.shot then
		...
		if not self.fall then
			...
			--stop, hammertime
			if self.hammers and self.backwards == false and (onlinemp and clientisnetworkhost or not onlinemp) then
				self.hammertimer = self.hammertimer + dt
				while self.hammertimer > self.hammertime do
					table.insert(networksendqueue, "hammer;" .. networkclientnumber .. ";" .. self.x+4/16 .. ";" .. self.y + .5 .. ";left")
		...
		if not self.fall and (onlinemp and clientisnetworkhost or not onlinemp) then
			if objects["player"][getclosestplayer(self.x+15/16)].x > self.x+15/16 and self.jump == false then
				...
				if self.timer > bowserjumpdelay and self.jump == false then
					...
					table.insert(networksendqueue, "bowserjump;" .. networkclientnumber .. ";" .. self.speedy)
				end
			end
		end
	end
...
function bowser:newtargetx(dir, reason)
	if (onlinemp and clientisnetworkhost or not onlinemp) or reason == "initial" then
		if dir == "right" then
