...
function rocketlauncher:update(dt)
	if (onlinemp and clientisnetworkhost or not onlinemp) then
		local onscreen
		for x = 1, players do
			local v = objects["player"][x]
			if v.x > self.x - width*.75 and v.x < self.x + width*.35 then
				onscreen = true
				break
			end
		end
		self.timer = self.timer + dt
		if self.timer > self.time and onscreen then
			if self:fire() then
				self.timer = 0
				self:randomtime()
			end
...
function rocketlauncher:fire()
	...
	if objects["player"][pl].x + 14/16 > self.x + bulletbillrange then
		...
		table.insert(networksendqueue, "bullet;" .. networkclientnumber .. ";" .. self.x .. ";" .. self.y .. ";right")
		return true
	elseif objects["player"][pl].x + 14/16 < self.x - bulletbillrange then
		...
		table.insert(networksendqueue, "bullet;" .. networkclientnumber .. ";" .. self.x .. ";" .. self.y .. ";left")
		return true
	end
end
