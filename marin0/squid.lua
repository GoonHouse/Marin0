...
function squid:update(dt)
	...
	if self.shot then
		...
		if self.closestplayer then
			closestplayer = self.closestplayer
		end
		...
		if self.state == "idle" then
			...
			if (onlinemp and clientisnetworkhost or not onlinemp) and (self.y+self.speedy*dt) + self.height + 0.0625 >= (objects["player"][closestplayer].y - (24/16 - objects["player"][closestplayer].height)) then
				self.state = "upward"
				self.upx = self.x
				self.speedx = 0
				self.speedy = 0
				...
				if (onlinemp and clientisnetworkhost or not onlinemp) then
					for i, v in pairs(objects["squid"]) do
						if self == v then
							table.insert(networksendqueue, "squidstate;" .. networkclientnumber .. ";" .. i .. ";" .. self.state .. ";" .. self.direction .. ";" .. self.upx)
							break
						end
					end
				end
		...
		elseif self.state == "upward" then
			...
			if math.abs(self.x - self.upx) >= 2 and (onlinemp and clientisnetworkhost or not onlinemp) then
				self.state = "downward"
				...
				if (onlinemp and clientisnetworkhost or not onlinemp) then
					for i, v in pairs(objects["squid"]) do
						if self == v then
							table.insert(networksendqueue, "squidstate;" .. networkclientnumber .. ";" .. i .. ";" .. self.state .. ";" .. self.direction .. ";" .. self.downy)
							break
						end
					end
				end
		...
		elseif self.state == "downward" then
			self.speedy = squidfallspeed
			if self.y > self.downy + squiddowndistance and (onlinemp and clientisnetworkhost or not onlinemp) then
				self.state = "idle"
				self.quad = squidquad[1]
				if (onlinemp and clientisnetworkhost or not onlinemp) then
					for i, v in pairs(objects["squid"]) do
						if self == v then
							table.insert(networksendqueue, "squidstate;" .. networkclientnumber .. ";" .. i .. ";" .. self.state)
							break
						end
					end
				end
...
function squid:stomp()--maurice you butt, bloopers can be stomped
	self:shotted()
	self.speedx = 0
	self.speedy = 0
	self.quad = squidquad[2]
	if not relayed then
		table.insert(networksendqueue, "squidkill;" .. networkclientnumber .. ";" .. self.x .. ";" .. self.y .. ";stomp")
	end
end
