...
keyshiftvalueschat = {
	...
	{value=",", change="<"},
	{value=".", change=">"},
	{value="/", change="?"}, --THE ALMIGHTY QUESTION MARK
	{value="\'", change="\""},
	{value="[", change="{"},
	{value="]", change="}"}
...
function guielement:init(...)
	...
	elseif arg[1] == "input" then --input(x, y, width, enterfunc, start, maxlength)
		...
		self.backspaceoriginaltimer = 0
		self.backspacedeletetimer = 0
	end
end
	...
	if self.active then
		...
		elseif self.type == "input" then
			...
			if love.keyboard.isDown("backspace") then
				self.backspaceoriginaltimer = self.backspaceoriginaltimer + dt
				if self.backspaceoriginaltimer > .2 then
					self.backspacedeletetimer = self.backspacedeletetimer + dt
					if self.backspacedeletetimer > .1 then
...
function guielement:draw()
	...
	elseif self.type == "input" then
		...
		local newstring = {}
		if not self.usecoolvetica then
			...
			table.insert(newstring, oldstring)
		end
		
		for i = 1, #newstring do
			local stringtohide = newstring[i]
			if self.hidetext then
				local finalizedstring = ""
				for x = 1, #stringtohide do
					local character = string.sub(stringtohide, x, x)
					if character == "." then
						finalizedstring = finalizedstring .. "."
					else
						finalizedstring = finalizedstring .. "x"
					end

				end
				stringtohide = finalizedstring
			end
			if not self.usecoolvetica then
				properprint(stringtohide, (self.x+2)*scale, (self.y+2+1+(i-1)*10)*scale)
			else
				love.graphics.setFont(bigchatlogfont)
				love.graphics.print(stringtohide, (self.x+2)*scale, (self.y-2)*scale)
			end
		end
		...
		if self.height == 1 then
			if self.inputting and self.cursorblink then
				if not self.usecoolvetica then
					...
					love.graphics.rectangle("fill", (self.x+2)*scale+bigchatlogfont:getWidth(self.value), (self.y+10)*scale, 8*scale, 1*scale)
				end
			end
		end
	end
...
function guielement:keypress(key, unicode)
	if self.active then
			...
			if self.inputting then
				...
				elseif key == "backspace" then
					...
					if not self.usecoolvetica then
						...
						if bigchatlogfont:getWidth(self.value) < (1+self.width*8+2)*scale then
							pass = true
						end
					end
					...
					if pass then
						if not self.usecoolvetica then
							...
							if found then
								...
								--Numbers 							Capital letters						Small letter
							--if (unicode > 47 and unicode < 58) or (unicode > 64 and unicode < 91) or (unicode > 96 and unicode < 123) or key == " " then
							if (unicode > 32 and unicode < 127) or key == " " then
								...
								self.value = self.value .. keyvalue
								if self.cursorpos ~= self.maxlength then
									self.cursorpos = self.cursorpos + 1
								end
								if self.cursorpos > self.offset+self.width then
