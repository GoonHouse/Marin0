...
function seesaw:draw()
	--left
	love.graphics.draw(seesawimg, seesawquad[1], math.floor((self.x-1-xscroll)*16*scale), (self.y-1.5-yscroll)*16*scale, 0, scale, scale)
	...
	if self.falloff == false and self.leftplatform.y-self.y >= 0 then
		...
		for i = 1, math.ceil(self.leftplatform.y-self.y) do
			love.graphics.draw(seesawimg, seesawquad[3], math.floor((self.x-1-xscroll)*16*scale), (self.y+i-1.5-yscroll)*16*scale, 0, scale, scale)
		end
	...
	for i = 1, self.range-1 do
		love.graphics.draw(seesawimg, seesawquad[4], math.floor((self.x-1+i-xscroll)*16*scale), (self.y-1.5-yscroll)*16*scale, 0, scale, scale)
	end
		
	--right
	love.graphics.draw(seesawimg, seesawquad[2], math.floor((self.x-1+self.range-xscroll)*16*scale), (self.y-1.5-yscroll)*16*scale, 0, scale, scale)
	...
	if self.falloff == false and self.rightplatform.y-self.y >= 0 then
		...
		for i = 1, math.ceil(self.rightplatform.y-self.y) do
			love.graphics.draw(seesawimg, seesawquad[3], math.floor((self.x+self.range-1-xscroll)*16*scale), (self.y+i-1.5-yscroll)*16*scale, 0, scale, scale)
		end
		love.graphics.setScissor()
	else
